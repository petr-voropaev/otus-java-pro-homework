...
public class ATMService implements ATMOperation {
    ...
    public WithdrawalResult withdraw(int amount) {
        ...
        Map<Banknote, Integer> withdrawalResult = calculateWithdrawal(cells, amount);

        if (withdrawalResult.isEmpty()) {
            return WithdrawalResult.failure("Cannot withdraw amount");
        }

        removeBanknotesFromCells(withdrawalResult);

        return WithdrawalResult.success(withdrawalResult);
    }

    private Map<Banknote, Integer> calculateWithdrawal(Map<Integer, Cell> cells, int amount) {
    ...
    private void removeBanknotesFromCells(Map<Banknote, Integer> withdrawal) {
